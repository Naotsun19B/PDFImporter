...
class FCanvas;
class IPDFViewerToolkit;
class SPDFViewerViewport;
class UTexture2D;

class FPDFViewerViewportClient
	: public FViewportClient
	, public FGCObject
{
public:
	/** Constructor */
	FPDFViewerViewportClient(TWeakPtr<IPDFViewerToolkit> InPDFViewer, TWeakPtr<SPDFViewerViewport> InPDFViewerViewport);
	~FPDFViewerViewportClient();
...
private:
	...
	TWeakPtr<IPDFViewerToolkit> PDFViewerPtr;

	/** Pointer back to the Texture viewport control that owns us */
	TWeakPtr<SPDFViewerViewport> PDFViewerViewportPtr;
