...
#include "Models/PDFViewerViewportClient.h"
...
/* FPDFViewerViewportClient structors
 *****************************************************************************/

FPDFViewerViewportClient::FPDFViewerViewportClient( TWeakPtr<IPDFViewerToolkit> InPDFViewer, TWeakPtr<SPDFViewerViewport> InPDFViewerViewport )
	: PDFViewerPtr(InPDFViewer)
	, PDFViewerViewportPtr(InPDFViewerViewport)
	, CheckerboardTexture(NULL)
{
	check(PDFViewerPtr.IsValid() && PDFViewerViewportPtr.IsValid());

	ModifyCheckerboardTextureColors();
}


FPDFViewerViewportClient::~FPDFViewerViewportClient( )
{
...
void FPDFViewerViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
	if (!PDFViewerPtr.IsValid())
	{
		return;
	}
	
	UTexture* Texture = PDFViewerPtr.Pin()->GetTexture();
	FVector2D Ratio = FVector2D(GetViewportHorizontalScrollBarRatio(), GetViewportVerticalScrollBarRatio());
	FVector2D ViewportSize = FVector2D(PDFViewerViewportPtr.Pin()->GetViewport()->GetSizeXY().X, PDFViewerViewportPtr.Pin()->GetViewport()->GetSizeXY().Y);
	...
	PDFViewerPtr.Pin()->PopulateQuickInfo();
...
bool FPDFViewerViewportClient::InputKey(FViewport* Viewport, int32 ControllerId, FKey Key, EInputEvent Event, float AmountDepressed, bool Gamepad)
{
	if (Key == EKeys::MouseScrollUp)
	{
		PDFViewerPtr.Pin()->ZoomIn();

		return true;
	}
	else if (Key == EKeys::MouseScrollDown)
	{
		PDFViewerPtr.Pin()->ZoomOut();

		return true;
	}
	else if (Key == EKeys::RightMouseButton)
	{
		PDFViewerPtr.Pin()->SetVolumeOrientation(FRotator(90, 0, -90));
	}
	return false;
}

bool FPDFViewerViewportClient::InputAxis(FViewport* Viewport, int32 ControllerId, FKey Key, float Delta, float DeltaTime, int32 NumSamples, bool bGamepad)
{
	if (Key == EKeys::MouseX || Key == EKeys::MouseY)
	{
		...
		PDFViewerPtr.Pin()->SetVolumeOrientation((FRotationMatrix::Make(DeltaRotator) * FRotationMatrix::Make(PDFViewerPtr.Pin()->GetVolumeOrientation())).Rotator());
		return true;
	}
...
bool FPDFViewerViewportClient::InputGesture(FViewport* Viewport, EGestureEvent GestureType, const FVector2D& GestureDelta, bool bIsDirectionInvertedFromDevice)
{
	...
	{
		double CurrentZoom = PDFViewerPtr.Pin()->GetZoom();
		PDFViewerPtr.Pin()->SetZoom(CurrentZoom + GestureDelta.Y * 0.01);
		return true;
	}
...
void FPDFViewerViewportClient::AddReferencedObjects(FReferenceCollector& Collector)
{
	Collector.AddReferencedObject(CheckerboardTexture);
}


void FPDFViewerViewportClient::ModifyCheckerboardTextureColors()
{
	DestroyCheckerboardTexture();
...
FText FPDFViewerViewportClient::GetDisplayedResolution() const
{
	uint32 Height = 1;
	uint32 Width = 1;
	PDFViewerPtr.Pin()->CalculateTextureDimensions(Width, Height);
	return FText::Format( NSLOCTEXT("PDFViewer", "DisplayedResolution", "Displayed: {0}x{1}"), FText::AsNumber( FMath::Max((uint32)1, Width) ), FText::AsNumber( FMath::Max((uint32)1, Height)) );
}


float FPDFViewerViewportClient::GetViewportVerticalScrollBarRatio() const
{
	uint32 Height = 1;
	uint32 Width = 1;
	float WidgetHeight = 1.0f;
	if (PDFViewerViewportPtr.Pin()->GetVerticalScrollBar().IsValid())
	{
		PDFViewerPtr.Pin()->CalculateTextureDimensions(Width, Height);

		WidgetHeight = PDFViewerViewportPtr.Pin()->GetViewport()->GetSizeXY().Y;
	}
...
float FPDFViewerViewportClient::GetViewportHorizontalScrollBarRatio() const
{
	uint32 Width = 1;
	uint32 Height = 1;
	float WidgetWidth = 1.0f;
	if (PDFViewerViewportPtr.Pin()->GetHorizontalScrollBar().IsValid())
	{
		PDFViewerPtr.Pin()->CalculateTextureDimensions(Width, Height);

		WidgetWidth = PDFViewerViewportPtr.Pin()->GetViewport()->GetSizeXY().X;
	}
...
void FPDFViewerViewportClient::UpdateScrollBars()
{
	TSharedPtr<SPDFViewerViewport> Viewport = PDFViewerViewportPtr.Pin();
...
FVector2D FPDFViewerViewportClient::GetViewportScrollBarPositions() const
{
	FVector2D Positions = FVector2D::ZeroVector;
	if (PDFViewerViewportPtr.Pin()->GetVerticalScrollBar().IsValid() && PDFViewerViewportPtr.Pin()->GetHorizontalScrollBar().IsValid())
	{
		uint32 Width, Height;
		UTexture* Texture = PDFViewerPtr.Pin()->GetTexture();
		float VRatio = GetViewportVerticalScrollBarRatio();
		float HRatio = GetViewportHorizontalScrollBarRatio();
		float VDistFromBottom = PDFViewerViewportPtr.Pin()->GetVerticalScrollBar()->DistanceFromBottom();
		float HDistFromBottom = PDFViewerViewportPtr.Pin()->GetHorizontalScrollBar()->DistanceFromBottom();
	
		PDFViewerPtr.Pin()->CalculateTextureDimensions(Width, Height);

		if ((PDFViewerViewportPtr.Pin()->GetVerticalScrollBar()->GetVisibility() == EVisibility::Visible) && VDistFromBottom < 1.0f)
		{
		...
		if ((PDFViewerViewportPtr.Pin()->GetHorizontalScrollBar()->GetVisibility() == EVisibility::Visible) && HDistFromBottom < 1.0f)
		{
...
void FPDFViewerViewportClient::DestroyCheckerboardTexture()
{
