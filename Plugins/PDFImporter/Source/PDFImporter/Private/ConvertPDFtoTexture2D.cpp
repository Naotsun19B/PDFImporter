...
UConvertPDFtoTexture2D::UConvertPDFtoTexture2D(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer), WorldContextObject(nullptr), bIsActive(false), 
	  PDFFilePath(""),  Dpi(0), StartPage(0), EndPage(0)
{
...
void UConvertPDFtoTexture2D::ExecConversion()
{
	...
	// PDFファイルをコピー
	FString TargetFilePath = FPaths::Combine(TempDirPath, TEXT("ConvertTarget.pdf"));
	FileManager.Copy(*TargetFilePath, *PDFFilePath);
	if (!FileManager.FileExists(*TargetFilePath))
	{
		UE_LOG(PDFImporter, Error, TEXT("File couldn't be copied : %s"), *PDFFilePath);
		Failed.Broadcast();
		return;
	}

	//Ghostscriptを用いてPDFからjpg画像を作成
	std::string InputPath(TCHAR_TO_UTF8(*TargetFilePath));
	std::string OutputPath(TCHAR_TO_UTF8(*FPaths::Combine(TempDirPath, TEXT("%10d.jpg"))));
	...
	bool bResult = ConvertPdfToJpeg(InputPath, OutputPath, StartPage, EndPage, Dpi);
	...
	if (bResult)
	{
		...
		{
			UE_LOG(PDFImporter, Warning, TEXT("%s"), *PageName);
