...
UGhostscriptCore* UGhostscriptCore::Instance = nullptr;

void UGhostscriptCore::CreateModule()
{
	if (Instance == nullptr)
	{
		Instance = NewObject<UGhostscriptCore>();
		check(Instance);
		Instance->AddToRoot();
	}
}

void UGhostscriptCore::DestroyModule()
{
	if (Instance != nullptr)
	{
		Instance = nullptr;
...
UGhostscriptCore* UGhostscriptCore::Get()
{
	if (Instance == nullptr)
	{
		UE_LOG(PDFImporter, Fatal, TEXT("Module not loaded : UGhostscript::CreateModule must be executed"));
	}

	return Instance;
}

UGhostscriptCore::UGhostscriptCore()
{
	//dllファイルのパスを取得
...
void UGhostscriptCore::BeginDestroy()
{
	Super::BeginDestroy();
...
UPDF* UGhostscriptCore::ConvertPdfToPdfAsset(const FString& InputPath, int Dpi, int FirstPage, int LastPage, const FString& Locale)
{
...
bool UGhostscriptCore::ConvertPdfToJpeg(const FString& InputPath, const FString& OutputPath, int Dpi, int FirstPage, int LastPage, const FString& Locale)
{
...
bool UGhostscriptCore::LoadTexture2DFromFile(const FString& FilePath, UTexture2D* &LoadedTexture)
{
	//配列に画像を読み込む
...
int UGhostscriptCore::GetFStringSize(const FString& InString)
{
	int Size = 0;
